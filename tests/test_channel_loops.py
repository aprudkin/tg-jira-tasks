...
Гоняем настоящий _channel_loop через start()/stop() (моки только у Jira/Telegram) —
...
@pytest.mark.asyncio
async def test_two_channel_loops_tick_deliver_and_persist(isolated_state, monkeypatch):
    # Первая проверка почти сразу; интервал огромный, чтобы тикнуло ровно по разу
    monkeypatch.setattr(nots, "FIRST_CHECK_DELAY_SECONDS", 0.02)
    ...
    monkeypatch.setattr(
        "bot.services.notifications.jira_service.get_events_since", fake_events
    )
    ...
    fresh = nots.NotificationService()
...
@pytest.mark.asyncio
async def test_concurrent_saves_produce_valid_state(isolated_state):
    s = nots.NotificationService()
    s._chat_id = 100
